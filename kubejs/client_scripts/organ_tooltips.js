...
ItemEvents.tooltip((tooltip) => {
      ...
      switch (true) {
        ...
        default:
          ...
          if (organ.shiftTextLines && organ.shiftTextLines.length != 0) {
            text.add(lineNum++, [
              Text.of(
                Text.translatable("kubejs.tooltips.organ_score.3")
              ).gold(),
              Text.of(Text.translatable("kubejs.tooltips.organ_score.4"))
                .yellow()
                .bold(),
              Text.of(
                Text.translatable("kubejs.tooltips.organ_score.5")
              ).gold(),
            ]);
          }
          if (organ.ctrlTextLines && organ.ctrlTextLines.length != 0) {
            text.add(lineNum++, [
              ...
              Text.of(Text.translatable("kubejs.tooltips.organ_score.6"))
                .yellow()
                .bold(),
              Text.of(
                Text.translatable("kubejs.tooltips.organ_score.7")
              ).aqua(),
            ]);
          }
          if (organ.altTextLines && organ.altTextLines.length != 0) {
            text.add(lineNum++, [
              ...
              Text.of(Text.translatable("kubejs.tooltips.organ_score.8"))
                .yellow()
                .bold(),
  ...
  tooltip.addAdvanced("kubejs:infinity_force", (item, advanced, text) => {
    text.add(1, [
      Text.gold(Text.translatable("item.kubejs.infinity_force")),
      Text.yellow(` +${item.nbt?.forgeTimes ? item.nbt.forgeTimes : 0}`),
    ]);
  });
  ...
  tooltip.addAdvanced("kubejs:glass_vial", (item, advanced, text) => {
    ...
    item.nbt.organSocres.getAllKeys().forEach((key) => {
      text.add(lineNum++, [
        LEADING_SYMBOL,
        Text.yellow(global.SCORE_MAP[key]),
        Text.white(" : "),
        Text.white(item.nbt.organSocres[key].toFixed(2)),
      ]);
    });
  });
});
