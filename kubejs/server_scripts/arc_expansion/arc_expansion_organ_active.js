...
const dlcTemplateOrganActiveStrategies = {
  // 示例器官-激活
  "arc_expansion:example_organ_active": function (player, organ, attributeMap) {
    // 获取玩家的胸腔属性
    ...
    playerChestInstance.organScores.put(
      ...
      new $Float(
        playerChestInstance
          .getOrganScores()
          .get(new ResourceLocation("chestcavity", "health")) + 2
      )
    );
  },
};
...
var assign_organ_active = Object.assign(
  organActiveStrategies,
  dlcTemplateOrganActiveStrategies
);
...
const dlcTemplateOrganActiveOnlyStrategies = {
  // 示例器官-唯一激活
  "arc_expansion:example_organ_active_only": function (
    player,
    organ,
    attributeMap
  ) {
    ...
    if (typeMap.has("kubejs:heart")) {
      // 将属性修饰符添加到属性修饰符表中
      // 修饰符注册于startup_scripts/arc_expansion/arc_expansion_global中
      // 后者为具体要增加的值，typeMap.get('kubejs:heart').length为 心脏 类型器官的个数
      // 如果要减少，则使用负数
      attributeMapValueAddition(
        attributeMap,
        global.arc_expansion_HEALTH_UP_MULTI_BASE,
        typeMap.get("kubejs:heart").length * 0.02
      );
    }
  },
};

var assign_organ_active_only = Object.assign(
  organActiveOnlyStrategies,
  dlcTemplateOrganActiveOnlyStrategies
);
